...
class CloseDrawerEnv(BaseEnv):
    cfg: CloseDrawerEnvCfg

    def __init__(self, cfg: CloseDrawerEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.drawer_handle_offset = torch.tensor([-0.31, 0.3, -0.05], device=self.device)
    ...
    def _get_subtask(self):
        ...
        drawer_non_full_open_limit = drawer_bottom_lower + 0.1 * (drawer_bottom_upper - drawer_bottom_lower)
        self.move_door_success[torch.where(self.drawer.data.joint_pos[:, self.drawer_bottom_idx] > drawer_non_full_open_limit)] = 1
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        if self.cfg.randomize: 
            if self.cfg.randomize_idx < 0:
                drawer_root_state[:, :2] += (0.2 * self.cfg.randomize_range) * torch.rand((self.num_envs, 2), device=self.device) - (0.1 * self.cfg.randomize_range)
            else:
    ...
    def _get_observations(self) -> dict:
        ...
        self._get_subtask()
        obs["success"] = self.is_drawer_close
